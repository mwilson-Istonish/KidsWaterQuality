...
namespace CDPHE.H20.Data.Queries
{
    public static class RequestDetailQuery
    {
        ...
        {
            string sql = "SELECT Request.Id, Request.Status, Request.CreatedAt, Facility.Name AS Facility, [User].FirstName + ' ' + [User].LastName AS Provider, [User].Email, [User].Phone, Facility.WQCID, Facility.Address1, Facility.Address2, Facility.City, Facility.State, Facility.ZipCode FROM Request INNER JOIN Facility ON Request.FacilityId = Facility.Id INNER JOIN [User] ON Request.Id = [User].id WHERE Request.Id = @ID" +
        ...
        {
            string sql = "SELECT Request.Id AS Id, Facility.Name as Facility, [User].FirstName + ' ' + [User].LastName AS Provider, [User].Email, Request.CreatedAt, Request.Status FROM Request INNER JOIN Facility ON Request.FacilityId = Facility.Id INNER JOIN [User] ON Request.Id = [User].id WHERE Request.UserId = @Id";
            return sql;
        }
