...
namespace CDPHE.H20.Data.Queries
{
    public static class RequestDetailQuery
    {
        ...
        {
            string sql = "SELECT Request.Id, Request.Status, Request.CreatedAt, Facility.Id as FacilityId, Facility.Name AS FacilityName, [User].FirstName + ' ' + [User].LastName AS Provider, [User].Email, [User].Phone, Facility.WQCID, Facility.Address1, Facility.Address2, Facility.City, Facility.State, Facility.ZipCode FROM Request INNER JOIN Facility ON Request.FacilityId = Facility.Id INNER JOIN [User] ON Request.UserId = [User].id WHERE Request.Id = @ID" +
